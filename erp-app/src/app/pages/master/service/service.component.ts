...
export class ServiceComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    const obs = this.isEditMode ? this.masterSvc.updateService(this.selectedId, this.form) : this.masterSvc.createService(this.form);
  ...
  confirmDelete(): void {
    ...
    this.masterSvc.deleteService(this.itemToDelete.id).subscribe({ next: () => { this.showDeleteModal = false; this.itemToDelete = null; this.load(); }, error: () => { this.message = 'Delete failed.'; this.isError = true; this.showDeleteModal = false; } });
  }
}
