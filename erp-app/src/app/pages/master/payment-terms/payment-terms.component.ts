...
export class PaymentTermsComponent implements OnInit {
  ...
  confirmDelete(): void {
    ...
    this.masterSvc.deletePaymentTerm(this.itemToDelete.id).subscribe({ next: () => { this.showDeleteModal = false; this.itemToDelete = null; this.load(); }, error: () => { this.message = 'Delete failed.'; this.isError = true; this.showDeleteModal = false; } });
  }
}
