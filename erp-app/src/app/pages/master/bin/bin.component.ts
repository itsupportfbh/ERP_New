...
const initialFormState = { binName: '', description: '' };
...
export class BinComponent implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.masterSvc.getBins().subscribe({
  ...
  edit(item: any): void {
    ...
    this.form = { binName: item.binName || '', description: item.description || '' };
    this.message = '';
  }
  ...
  onSubmit(): void {
    if (!this.form.binName?.trim()) { this.message = 'Bin Name is required.'; this.isError = true; return; }
    const obs = this.isEditMode ? this.masterSvc.updateBin(this.selectedId, this.form) : this.masterSvc.createBin(this.form);
    obs.subscribe({
  ...
  confirmDelete(): void {
    ...
    this.masterSvc.deleteBin(this.itemToDelete.id).subscribe({
